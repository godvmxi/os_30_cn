...
void HariMain(void)
{
	...
	/* s是输出缓冲区 	keybuf是键盘缓冲区 	mousebuf是鼠标缓冲区 timerbuf*是定时器的缓冲区 */
	...
	timer_settime(timer, 1000);				/* 定时器设置 */
	...
	for (;;) {
		...
		sheet_refresh(sht_win, 40, 28, 120, 44);		/* 在屏幕的窗口中显示count的值 */

		io_cli();			/* 关闭所有可屏蔽中断 */
			/* 如果键盘、鼠标、3个定时器缓冲区都没有数据的话 */
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo)
			...
			if (fifo8_status(&keyfifo) != 0) {		/* 如果键盘缓冲区中有数据 */
				...
				sheet_refresh(sht_back, 0, 64, 56, 80);	/* 在屏幕上显示"10[sec]" */
			} else if (fifo8_status(&timerfifo2) != 0) {	/* 定时器2有数据 */
				i = fifo8_get(&timerfifo2); /* 读入数据 */
				io_sti();				/* 打开所有可屏蔽中断 */
				putfonts8_asc(buf_back, binfo->scrnx, 0, 80, COL8_FFFFFF, "3[sec]");
				sheet_refresh(sht_back, 0, 80, 48, 96);		/* 在屏幕上显示"3[sec]" */
			} else if (fifo8_status(&timerfifo3) != 0) {	/* 模拟光标的闪烁 */
				i = fifo8_get(&timerfifo3);		/* 读入数据 */
				io_sti();						/* 打开所有可屏蔽中断 */
				if (i != 0) {
					timer_init(timer3, &timerfifo3, 0); /* 重新设置定时器 此时设置data = 0 */
					boxfill8(buf_back, binfo->scrnx, COL8_FFFFFF, 8, 96, 15, 111); /* 描绘一个小矩形 */
				} else {
					timer_init(timer3, &timerfifo3, 1); /* 重新设置定时器 此时设置data = 1 */
					boxfill8(buf_back, binfo->scrnx, COL8_008484, 8, 96, 15, 111);	/* 描绘一个小矩形 */
				}
				timer_settime(timer3, 50);	/* 设置半秒 */
				sheet_refresh(sht_back, 8, 96, 16, 112);	/* 在屏幕上显示那个小矩形 */
			}
		}
	}
}
