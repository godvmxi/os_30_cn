...
void HariMain(void)
{
	...
	buf_back  = (unsigned char *) memman_alloc_4k(memman, binfo->scrnx * binfo->scrny);	/* 分配内存空间 用于绘制"桌面" */
	...
	sheet_slide(shtctl, sht_back, 0, 0);	/* 设置"桌面"图层的位置 */
	mx = (binfo->scrnx - 16) / 2; /* 计算鼠标图形在屏幕上的位置 它在整个桌面的中心位置 */
	my = (binfo->scrny - 28 - 16) / 2;	
	sheet_slide(shtctl, sht_mouse, mx, my);	/* 设置鼠标图层的位置 */
	sheet_updown(shtctl, sht_back,  0);		/* 调整"桌面"图层和鼠标图层的高度 */
	sheet_updown(shtctl, sht_mouse, 1);		/* 并且会显示"桌面"与鼠标图层 */
	...
	sheet_refresh(shtctl, sht_back, 0, 0, binfo->scrnx, 48);	/* 刷新sht_back图层的某一矩形区域
	...
	for (;;) {
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0) {	/* 如果键盘缓冲区或者鼠标缓冲区中都没有数据 */
			...
			if (fifo8_status(&keyfifo) != 0) {		/* 如果键盘缓冲区中有数据 */
				...
				sheet_refresh(shtctl, sht_back, 0, 16, 16, 32);		/* 刷新sht_back图层缓冲区中的某一矩形区域 */
				...
				if (mouse_decode(&mdec, i) != 0) {	/* 接收鼠标发送的数据 */
					...
					boxfill8(buf_back, binfo->scrnx, COL8_008484, 32, 16, 32 + 15 * 8 - 1, 31);	/* 以下两句都是对缓冲区中的数据进行更改 */
					putfonts8_asc(buf_back, binfo->scrnx, 32, 16, COL8_FFFFFF, s);	
					sheet_refresh(shtctl, sht_back, 32, 16, 32 + 15 * 8, 32);	/* 刷新sht_back图层缓冲区中的某一矩形区域以显示上面的更改 */
					mx += mdec.x;					/* 更新新的鼠标位置 */
					my += mdec.y;
					...
					sheet_refresh(shtctl, sht_back, 0, 0, 80, 16);			/* 刷新sht_back图层缓冲区中的某一矩形区域以显示上面的更改 */
