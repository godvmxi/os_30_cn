...
#define SHEET_USE		1			/* 标识图层正在使用符号常量 */
...
struct SHEET *sheet_alloc(struct SHTCTL *ctl)
{
	...
	int i;
	for (i = 0; i < MAX_SHEETS; i++) {
		if (ctl->sheets0[i].flags == 0) {	/* 遍历整个sheets0数组查找第一个未使用的图层 */
			sht = &ctl->sheets0[i];
			sht->flags = SHEET_USE; /* 标识为正在使用 */
			sht->height = -1; 	/* 隐藏 高度为-1 其实就是表示该图层不在屏幕上显示 */
			sht->task = 0;		/* 先将该变量置0 由调用者自行赋值 */	
			return sht;
		}
	}
	return 0;	
...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
	...
	for (h = h0; h <= ctl->top; h++) {		/* 遍历从h0到top的所有图层 */
		sht = ctl->sheets[h];
		sid = sht - ctl->sheets0; /* sid是图层在sheets0中的下标 */
		buf = sht->buf;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
	...
	for (h = h0; h <= h1; h++) {	/* 遍历图层 */
		...
		for (by = by0; by < by1; by++) {
			...
			for (bx = bx0; bx < bx1; bx++) {
				vx = sht->vx0 + bx;
				if (map[vy * ctl->xsize + vx] == sid) {	/* 如果该坐标的map值与sid相等 才重画 */
...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) {	/* 原来的高度比新高度还要大 */
		if (height >= 0) {
			/* 把中间的往上提 */
			for (h = old; h > height; h--) {	/* 我们可以认为高度就是图层在sheets数组中的下标 */
			...
			if (ctl->top > old) {	/* 如果原来的图层不是最上层的图层 */
				/* 把上面的降下来 */
	...
	} else if (old < height) {	/* 原来的高度比新高度还要小 */
		if (old >= 0) {			/* 原来的图层如果不是隐藏的 */
			/* 把中间的拉下去 */
