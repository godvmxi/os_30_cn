...
{
	struct TASK *task = task_now();			/* task标识控制台任务本身 */
	...
	*((int *) 0x0fec) = (int) &cons;
	...
	for (;;) {
		...
		if (fifo32_status(&task->fifo) == 0) {	/* 控制台任务自己的缓冲区中没有数据 */
			...
			if (i <= 1) {						/* 光标用定时器 */
				if (i != 0) {
					timer_init(cons.timer, &task->fifo, 0); /* 下面是设定0 */
					if (cons.cur_c >= 0) {
						cons.cur_c = COL8_FFFFFF;
					}
				} else {
					timer_init(cons.timer, &task->fifo, 1); /* 下面是设定1 */
					if (cons.cur_c >= 0) {
						cons.cur_c = COL8_000000;
					}
				}
				timer_settime(cons.timer, 50);	/* 设置半秒 */	
			}
			if (i == 2) {	/* 光标ON */
				cons.cur_c = COL8_FFFFFF;
			}
			...
			if (256 <= i && i <= 511) { /* 任务A传送过来的键盘的数据 */
				if (i == 8 + 256) {		/* 注意！在任务A中已经将键值转换成字符的ASCII码了 */
					...
					cons_newline(&cons);
					cons_runcmd(cmdline, &cons, fat, memtotal);	/* 执行命令或任务 */
					cons_putchar(&cons, '>', 1);
				} else {
					/* 一般字符 */
					if (cons.cur_x < 240) {
	...
	/************************************************************/
		...
		if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			...
			*((int *) 0xfe8) = (int) q;
			...
			/* 当应用程序执行结束之后会回到此处继续执行该函数 我们就判断应用程序的窗口 */
			/* 有没有关闭，如果没关闭就把它关掉 */
			shtctl = (struct SHTCTL *) *((int *) 0x0fe4);
			for (i = 0; i < MAX_SHEETS; i++) {
				sht = &(shtctl->sheets0[i]);
				if (sht->flags != 0 && sht->task == task) {	/* task是我们在SHEET结构中新加的一个成员变量 */
															/* 表示我们这个图层是属于哪个任务的 */
					/* 找到应用程序遗留的窗口 */
					sheet_free(sht);	/* 关闭 */
				}
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	struct SHEET *sht;
	int *reg = &eax + 1;	/* 啊。。。这里参数传递真心好奇怪啊！ */
							/* 参数应该是从右往左压栈 reg不是应该是&edi吗？这样reg[7]才会是eax啊 */
							/* 作者对于C语言和汇编语言之间调用的参数传递方式没有讲清楚 */
							/* 由于汇编器又是作者自己编写的 所以没地方查资料。 */
							/* 好吧 姑且就不去想这些了。。。 */
	...
	if (edx == 1) {			/* 初始单个字符 */
		...
		sheet_updown(sht, 3);	
		reg[7] = (int) sht;
	} else if (edx == 6) {	/* 在窗口中显示字符 */
		sht = (struct SHEET *) (ebx & 0xfffffffe);
		putfonts8_asc(sht->buf, sht->bxsize, esi, edi, eax, (char *) ebp + ds_base);
		if ((ebx & 1) == 0) {
			sheet_refresh(sht, esi, edi, esi + ecx * 8, edi + 16);
		}
	} else if (edx == 7) {	/* 在窗口中描绘方块 */
		sht = (struct SHEET *) (ebx & 0xfffffffe);
		boxfill8(sht->buf, sht->bxsize, ebp, eax, ecx, esi, edi);
		if ((ebx & 1) == 0) {
			sheet_refresh(sht, eax, ecx, esi + 1, edi + 1);
		}
	} else if (edx == 8) {	/* memmam初始化 */
